...
* **シジル**: `add`, `sub`, `sin` などの演算命令 などの魔法効果を象形文字として配置。
...
Unity標準のParticle Systemを、スクリプトから動的に構築します。プリセットを再生するのではなく、**すべてのパラメータ（発生数、形状、ライフタイム、ノイズなど）をランタイムで生成**するため、無限のバリエーションを作成可能です。
...
2. **ローカルサーバーの起動**:
   ...
   * `index.html` があるディレクトリでサーバーを立ち上げます。
