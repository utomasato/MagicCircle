...
function magicTemplates(magic, spell)
{
    ...
    {
        case "fire":
            convertedSpell = `{ dict begin ~preset < ~main < ~startLifetime [ 0.5 2 ] ~startSpeed 0.5 ~startSize [ 0.2 0.4 ] ~startRotation [ 0 360 ] > ~emission < ~rateOverTime 50 > ~shape < ~angle 5 ~radius 0.0001 > ~colorOverLifetime < ~gradient < ~colorKeys [ [ ${prms.color1} 0.0 ] [ ${prms.color2} 0.6 ] [ ${prms.color2} 1.0 ] ] ~alphaKeys [ [ 0.0 0.0 ] [ 1.0 0.5 ] [ 0.0 1.0 ] ] > > ~rotationOverLifetime < ~z [ -45 45 ] > ~renderer < ~materialName (Fire_1) > > def ~magic $preset magicactivate $magic <${prmfgs.position ? " ~position [ " + prms.position + " ]": ""} ~rotation [ ${prms.rotation} ] ~scale ${prms.scale} > transform end }`;
            break;
        case "bullet":
